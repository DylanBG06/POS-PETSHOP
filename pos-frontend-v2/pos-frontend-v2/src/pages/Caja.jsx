...
import { Banknote, Smartphone, CreditCard, AlertCircle, Check, Calendar, Clock, Trash2 } from 'lucide-react'
...
export default function Caja() {
  ...
  const eliminarCierre = async (cierre) => {
    if (!confirm(`¿Eliminar cierre del ${formatFechaHora(cierre.fecha_cierre)}? Los datos del resumen se recalcularán.`)) return
    try {
  ...
  if (loading || !resumen) return <div className="text-center py-12 text-slate-400">Cargando...</div>
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        <div className="card">
          ...
          <div className="text-xs text-slate-500 mt-2 flex items-center gap-1">
            <Clock size={12} /> {new Date().toLocaleTimeString('es-CR', { hour: '2-digit', minute: '2-digit' })}
          </div>
        </div>
      </div>
      ...
      <div className="card">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          <MetodoPagoCard icon={Smartphone} label="SINPE Móvil" valor={resumen.total_sinpe} total={resumen.total_dia} color="brand" />
      ...
      <div className="card bg-gradient-to-br from-amber-50 to-orange-50 border-amber-200">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            ...
            <div className="text-xs text-amber-800 mt-1">Al cerrar, los valores se resetean y las ventas posteriores contarán para el siguiente turno.</div>
          </div>
            ...
            <tbody className="divide-y divide-slate-100">
              {ventas.length === 0 ? (
                ...
                <tr key={v.id} className="hover:bg-slate-50/80">
                  <td className="py-2 px-3 text-xs text-slate-600 tabular-nums">{new Date(v.fecha).toLocaleTimeString('es-CR',{hour:'2-digit',minute:'2-digit'})}</td>
              ...
              <div key={c.id} className="grid grid-cols-[1fr_90px_90px_110px_30px] gap-2 items-center py-2 border-b border-slate-100 last:border-0 text-sm">
                ...
                <div className={`text-right px-2 py-1 rounded-md ${
                  ...
                  <div className="font-semibold flex items-center justify-end gap-1">
                    {c.diferencia===0 ? <><Check size={14} /> OK</> : c.diferencia>0 ? `+${formatColones(c.diferencia)}` : formatColones(c.diferencia)}
                  </div>
                </div>
                <button onClick={() => eliminarCierre(c)} className="text-red-400 hover:text-red-600 p-1" title="Eliminar cierre">
                  <Trash2 size={14} />
                </button>
              </div>
            ))}
      ...
      <CierreModal open={modalCierre} onClose={() => setModalCierre(false)} resumen={resumen} onCerrar={() => { setModalCierre(false); cargar() }} />
...
function MetodoPagoCard({ icon: Icon, label, valor, total, color }) {
  ...
  const c = { emerald: { bg:'bg-emerald-50',text:'text-emerald-700',bar:'bg-emerald-500',icon:'text-emerald-600' }, brand: { bg:'bg-brand-50',text:'text-brand-700',bar:'bg-brand-500',icon:'text-brand-600' }, blue: { bg:'bg-blue-50',text:'text-blue-700',bar:'bg-blue-500',icon:'text-blue-600' } }[color]
  return (
    <div className={`${c.bg} rounded-xl p-3`}>
      ...
      <div className="bg-white/60 h-1.5 rounded-full mt-2 overflow-hidden"><div className={`${c.bar} h-full rounded-full`} style={{width:`${pct}%`}} /></div>
    </div>
  )
}
...
function CierreModal({ open, onClose, resumen, onCerrar }) {
  ...
  const cerrar = async () => {
    ...
    setCerrando(true)
    try { await cajaAPI.cerrar({ total_real: real, notas: notas.trim() || null }); toast.success('Cierre registrado — valores reseteados'); onCerrar() }
    catch (err) { toast.error(err.message) }
    ...
    <Modal open={open} onClose={onClose} title="Cierre de caja">
      <div className="space-y-4">
        <div className="bg-slate-50 rounded-lg p-3 space-y-2 text-sm">
          <div className="flex justify-between"><span className="text-slate-500">Ventas desde último cierre</span><span className="font-medium">{resumen.cantidad_ventas}</span></div>
        ...
        <div>
          ...
          <input type="number" value={totalReal} onChange={e => setTotalReal(e.target.value)} placeholder="0" className="input-base w-full text-2xl text-right font-medium py-3" autoFocus />
        </div>
        {totalReal !== '' && (
          <div className={`rounded-lg p-3 ${diferencia===0?'bg-emerald-50 text-emerald-800':diferencia>0?'bg-blue-50 text-blue-800':'bg-red-50 text-red-800'}`}>
            <div className="text-xs mb-1">Diferencia</div>
            <div className="text-xl font-bold flex items-center gap-2">
              {diferencia===0 ? <><Check size={18} /> Cuadrado</> : diferencia>0 ? `+${formatColones(diferencia)} (sobrante)` : `${formatColones(diferencia)} (faltante)`}
            </div>
          </div>
        )}
        <div><label className="text-xs text-slate-500 mb-1 block">Notas (opcional)</label><textarea value={notas} onChange={e => setNotas(e.target.value)} placeholder="Observaciones..." className="input-base w-full" rows={2} /></div>
        <div className="flex justify-end gap-2">
          <button onClick={onClose} className="btn-secondary">Cancelar</button>
          <button onClick={cerrar} disabled={cerrando || totalReal===''} className="btn-primary disabled:opacity-50">{cerrando ? 'Guardando...' : 'Confirmar cierre'}</button>
        </div>
      </div>
    </Modal>
  )
      ...
      <div className="space-y-3">
        ...
        <div className="border-t border-slate-200 pt-3 space-y-1">
          {venta.metodo_pago==='efectivo' && venta.monto_recibido && (
            <>
              <div className="flex justify-between text-sm text-slate-500"><span>Recibido</span><span>{formatColones(venta.monto_recibido)}</span></div>
              <div className="flex justify-between text-sm text-slate-500"><span>Vuelto</span><span>{formatColones(venta.vuelto||0)}</span></div>
            </>
          )}
          <div className="flex justify-between items-baseline border-t border-slate-100 pt-2"><span className="text-sm text-slate-500">Total</span><span className="text-2xl font-bold text-brand-700">{formatColones(venta.total)}</span></div>
        </div>
      </div>
    </Modal>
  )
